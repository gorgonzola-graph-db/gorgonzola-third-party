...
static const bool DfaExtraDebug = false;
...
public:
    ...
    bool Search(const StringPiece& text, const StringPiece& context, bool anchored,
        bool want_earliest_match, bool run_forward, bool* failed, const char** ep,
        SparseSet* matches);
...
private:
    ...
    struct SearchParams {
        SearchParams(const StringPiece& text, const StringPiece& context, RWLocker* cache_lock)
            : text(text), context(context), anchored(false), can_prefix_accel(false),
              want_earliest_match(false), run_forward(false), start(NULL), cache_lock(cache_lock),
              failed(false), ep(NULL), matches(NULL) {}
...
public:
    ...
    Workq(int n, int maxmark)
        : SparseSet(n + maxmark), n_(n), maxmark_(maxmark), nextmark_(n), last_was_mark_(true) {}
...
DFA::DFA(Prog* prog, Prog::MatchKind kind, int64_t max_mem)
    : prog_(prog), kind_(kind), init_failed_(false), q0_(NULL), q1_(NULL), mem_budget_(max_mem) {
    if (DfaExtraDebug)
    ...
    int nstack = prog_->inst_count(kInstCapture) + prog_->inst_count(kInstEmptyWidth) +
                 prog_->inst_count(kInstNop) + nmark + 1; // + 1 for start inst

    // Account for space needed for DFA, q0, q1, stack.
    mem_budget_ -= sizeof(DFA);
    mem_budget_ -= (prog_->size() + nmark) * (sizeof(int) + sizeof(int)) * 2; // q0, q1
    ...
    for (Workq::iterator it = q->begin(); it != q->end(); ++it) {
        ...
        case kInstAltMatch:
            ...
            if (kind_ != Prog::kManyMatch &&
                (kind_ != Prog::kFirstMatch || (it == q->begin() && ip->greedy(prog_))) &&
...
DFA::State* DFA::CachedState(int* inst, int ninst, uint32_t flag) {
    ...
    int nnext = prog_->bytemap_range() + 1; // + 1 for kByteEndText slot
    int mem = sizeof(State) + nnext * sizeof(std::atomic<State*>) + ninst * sizeof(int);
...
void DFA::AddToQueue(Workq* q, int id, uint32_t flag) {
    ...
    Loop:
        ...
        case kInstNop:
            ...
            if (ip->opcode() == kInstNop && q->maxmark() > 0 && id == prog_->start_unanchored() &&
                id != prog_->start())
                stk[nstk++] = Mark;
            id = ip->out();
            goto Loop;
...
void DFA::RunWorkqOnByte(Workq* oldq, Workq* newq, int c, uint32_t flag, bool* ismatch) {
    // mutex_.AssertHeld();
    ...
    for (Workq::iterator i = oldq->begin(); i != oldq->end(); ++i) {
        ...
        case kInstMatch:
            if (prog_->anchor_end() && c != kByteEndText && kind_ != Prog::kManyMatch)
                break;
            *ismatch = true;
    ...
    if (DfaExtraDebug)
        fprintf(stderr, "%s on %d[%#x] -> %s [%d]\n", DumpWorkq(oldq).c_str(), c, flag,
            DumpWorkq(newq).c_str(), *ismatch);
}
...
inline bool DFA::InlinedSearchLoop(SearchParams* params) {
    ...
    const uint8_t* ep = BytePtr(params->text.data() + params->text.size()); // end of text
    ...
    while (p != ep) {
        ...
        if (ns == NULL) {
            ...
            if (ns == NULL) {
                ...
                if ((start = save_start.Restore()) == NULL || (s = save_s.Restore()) == NULL) {
...
bool DFA::FastSearchLoop(SearchParams* params) {
    ...
    int index =
        4 * params->can_prefix_accel + 2 * params->want_earliest_match + 1 * params->run_forward;
...
bool DFA::AnalyzeSearch(SearchParams* params) {
    ...
    // different state!
    if (prog_->can_prefix_accel() && !params->anchored && params->start > SpecialStateMax &&
    ...
    if (DfaExtraDebug)
        fprintf(stderr, "anchored=%d fwd=%d flags=%#x state=%s can_prefix_accel=%d\n",
            params->anchored, params->run_forward, flags, DumpState(params->start).c_str(),
...
bool DFA::AnalyzeSearchHelper(SearchParams* params, StartInfo* info, uint32_t flags) {
    // Quick check.
    ...
    q0_->clear();
    AddToQueue(q0_, params->anchored ? prog_->start() : prog_->start_unanchored(), flags);
...
bool DFA::Search(const StringPiece& text, const StringPiece& context, bool anchored,
    bool want_earliest_match, bool run_forward, bool* failed, const char** epp,
    SparseSet* matches) {
    *epp = NULL;
    if (!ok()) {
        *failed = true;
...
DFA* Prog::GetDFA(MatchKind kind) {
    ...
    if (kind == kFirstMatch) {
        std::call_once(
            dfa_first_once_,
            [](Prog* prog) { prog->dfa_first_ = new DFA(prog, kFirstMatch, prog->dfa_mem_ / 2); },
            this);
        return dfa_first_;
    } else if (kind == kManyMatch) {
        std::call_once(
            dfa_first_once_,
            [](Prog* prog) { prog->dfa_first_ = new DFA(prog, kManyMatch, prog->dfa_mem_); }, this);
        return dfa_first_;
    } else {
        std::call_once(
            dfa_longest_once_,
...
//
bool Prog::SearchDFA(const StringPiece& text, const StringPiece& const_context, Anchor anchor,
    MatchKind kind, StringPiece* match0, bool* failed, SparseSet* matches) {
    *failed = false;
    ...
    const char* ep;
    bool matched =
        dfa->Search(text, context, anchored, want_earliest_match, !reversed_, failed, &ep, matches);
    if (*failed) {
    ...
    if (match0) {
        if (reversed_)
            *match0 = StringPiece(ep, static_cast<size_t>(text.data() + text.size() - ep));
        else
            *match0 = StringPiece(text.data(), static_cast<size_t>(ep - text.data()));
    }
    return true;
}
...
int DFA::BuildAllStates(const Prog::DFAStateCallback& cb) {
    ...
    while (!q.empty()) {
        ...
        if (cb)
            cb(oom ? NULL : output.data(), s == FullMatchState || s->IsMatch());
        if (oom)
            break;
    }
